...
> 随机选取一些键进行删除；
> 
2.allkeys-random：随机删除所有键，直到腾出足够空间为止；</br>
...
4.allkeys-lru：根据 LRU 算法删除键，不管数据有没有设置超时属性，直到腾出足够空间为止。</br>
5.volatile-lru(默认): 超过最大内存后，在`过期键`中使用 lru 算法进行 key 的剔除，保证不过期数据不被删除，但是可能会出现 OOM 问题。</br>
